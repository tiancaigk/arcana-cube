...
function validateCubeData(cubeData) {
  ...
  if (cubeData.basicLands !== undefined && !Array.isArray(cubeData.basicLands)) {
    throw new Error("本地价格更新缺少有效基本地牌表");
  }
  const cards = requestedCards(cubeData);
  if (!cards.length) throw new Error("本地价格更新没有可处理的卡牌");
  if (cards.length > 5000) throw new Error("本地价格更新的卡牌数量异常");
  cards.forEach((card, index) => {
    if (!card || typeof card !== "object") throw new Error(`本地价格更新第 ${index + 1} 张卡格式无效`);
    if (!UUID_PATTERN.test(String(card.scryfallId || "").trim())) {
      throw new Error(`本地价格更新第 ${index + 1} 张卡缺少有效 Scryfall ID`);
    }
    if (!UUID_PATTERN.test(String(card.oracleId || "").trim())) {
      throw new Error(`本地价格更新第 ${index + 1} 张卡缺少有效 Oracle ID`);
    }
    if (!String(card.set || "").trim()) throw new Error(`本地价格更新第 ${index + 1} 张卡缺少系列`);
    if (!String(card.collectorNumber || "").trim()) throw new Error(`本地价格更新第 ${index + 1} 张卡缺少编号`);
  });
  return cubeData;
}
...
function validateBuiltIndex(index, expectedFingerprint, requestedCount) {
  ...
  const actualCount = index && index.cards && typeof index.cards === "object"
    ...
    : -1;
  if (!index || index.format !== "arcana-cube-mtgjson-prices" || Number(index.version) !== 2 || !Array.isArray(index.providers) || Array.isArray(index.cards)) {
    throw new Error("MTGJSON 本地索引构建结果格式无效");
  }
  if (!index.source || index.source.cubeFingerprint !== expectedFingerprint) {
    throw new Error("MTGJSON 本地索引与请求牌表不一致");
  }
  if (reportedCount !== requestedCount || !Number.isInteger(indexedCount) || indexedCount < 1 || indexedCount > requestedCount || actualCount !== indexedCount) {
    throw new Error("MTGJSON 本地索引构建结果数量异常");
  }
  return index;
}
...
function createLocalPriceIndexService(options = {}) {
  const rootDir = options.rootDir || path.resolve(__dirname, "..");
  const cacheDir = options.cacheDir || path.join(rootDir, ".cache", "mtgjson", "local");
  const builderFile = options.builderFile || path.join(rootDir, "scripts", "build-mtgjson-price-index.js");
  const cubeFile = options.cubeFile || path.join(cacheDir, "cube-data.json");
  const indexFile = options.indexFile || path.join(cacheDir, "mtgjson-price-index.json");
  const scriptFile = options.scriptFile || path.join(cacheDir, "mtgjson-price-index.js");
  ...
  async function build(cubeJson, fingerprint, cardCount) {
    ...
    try {
      ...
      try {
        await fsp.copyFile(indexFile, stagingIndexFile);
      } catch (error) {
      ...
      if (!scriptStat.isFile() || scriptStat.size < 1) throw new Error("MTGJSON 本地索引脚本构建结果无效");
...
module.exports = { createLocalPriceIndexService, runBuilder, validateBuiltIndex, validateCubeData };
